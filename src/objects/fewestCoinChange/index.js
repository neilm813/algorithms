...
/**
 * Calculates the fewest coins of the standard American denominations needed to reach the given cents amount.
 * - Time: O(?).
 * - Space: O(?).
 * @param {number} cents
 ...
 * - Space: O(1) constant, denominationTable has the same 4 keys (denominations) no matter how large cents gets.
 */
function fewestCoinChange(cents) {
...
/**
 * - Time: O(1) constant, the loop always loops over the 4 denominations regardless of how large cents is.
 * - Space: O(1) constant.
 */
function fewestCoinChange2(cents) {
  ...
  for (const denomination of denominationsDescending) {
    if (cents >= denomination.amount) {
      const count = Math.floor(cents / denomination.amount);
      cents -= count * denomination.amount;
